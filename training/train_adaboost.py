...
if __name__ == "__main__":
    print("[ADABOOST TRAINING] Loading dataset...")
    
  #  df = pd.read_parquet("data/task_a_trial.parquet")
    df = pd.read_parquet("data/train.parquet")
    df = df.dropna(subset=["code", "label"])
    
    print(f"[ADABOOST TRAINING] Dataset size: {len(df)} samples")
    ...
    print("[ADABOOST TRAINING] Extracting features...")
    X_list = []
    y_list = []
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if (idx + 1) % 100 == 0:
                print(f"[ADABOOST TRAINING] Processed {idx + 1}/{len(df)} samples...")
                
        except Exception as e:
            print(f"[ADABOOST TRAINING] Error processing row {idx}: {e}")
            continue
    
    X = np.array(X_list, dtype=np.float32)
    y = np.array(y_list, dtype=np.int32)
    
    print(f"[ADABOOST TRAINING] Final dataset: X shape={X.shape}, y shape={y.shape}")
    ...
    print(f"[ADABOOST TRAINING] Train size: {len(X_train)}, Test size: {len(X_test)}")
    
    # Train model
    print("[ADABOOST TRAINING] Training model...")
    model = AdaBoostStrategy()
    model.train(X_train, y_train)
    
    print("[ADABOOST TRAINING] Training complete!")
    
    # Evaluate
    print("[ADABOOST TRAINING] Evaluating model...")
    ...
    # Save model
    print("\n[ADABOOST TRAINING] Saving model...")
    model.save("data/adaboost.pkl")
    
    print("[ADABOOST TRAINING] ✓ Training complete! Model saved to data/adaboost.pkl")
